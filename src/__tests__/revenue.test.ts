...
describe('revenue.ts', () => {
  ...
  const jobs: Job[] = [
    { id: 1, name: 'カフェ', hourly_wage: 1000, employment_type: 'part', is_active: true },
    { id: 2, name: 'コンビニ', hourly_wage: 1100, employment_type: 'part', is_active: true },
  ];

  test('全月shiftsのみ（明細未入力）の場合、全てestimatedになる', () => {
    const shifts: Shift[] = [
      { id: 1, job_id: 1, date: '2026-01-10', start_time: '10:00', end_time: '15:00', break_minutes: 0, estimated_wage: 5000 },
      { id: 2, job_id: 2, date: '2026-02-15', start_time: '10:00', end_time: '12:00', break_minutes: 0, estimated_wage: 2200 },
    ];
  ...
  test('一部payslips・一部shifts混在の場合、payslipsが優先される', () => {
    ...
    const shifts: Shift[] = [
      { id: 1, job_id: 1, date: '2026-01-10', start_time: '10:00', end_time: '15:00', break_minutes: 0, estimated_wage: 5000 },
      { id: 2, job_id: 2, date: '2026-01-15', start_time: '10:00', end_time: '12:00', break_minutes: 0, estimated_wage: 2200 },
    ];
    // しかし1月はカフェの実績(5500)が入力されている。コンビニは実績未入力。
    const payslips: Payslip[] = [
      { id: 1, job_id: 1, year: 2026, month: 1, actual_amount: 5500 },
    ];
  ...
  test('全月payslips入力済みの場合、全てactualになる', () => {
    ...
    const payslips: Payslip[] = [
      { id: 1, job_id: 1, year: 2026, month: 1, actual_amount: 4800 },
      { id: 2, job_id: 2, year: 2026, month: 1, actual_amount: 2500 },
    ];

    const result = calcRevenue(baseSettings, jobs, shifts, payslips, currentYear);

    // シフトは無視され実績のみ合算される
    expect(result.annual_total).toBe(4800 + 2500);
  ...
  test('carryover_incomeが50万円の場合、年間合計に加算される', () => {
    ...
    const shifts: Shift[] = [];
    const payslips: Payslip[] = [
      { id: 1, job_id: 1, year: 2026, month: 1, actual_amount: 5000 },
    ];
